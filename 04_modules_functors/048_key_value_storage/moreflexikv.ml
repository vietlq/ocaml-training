...
module On_disk_table : TABLE with type param := string = struct
    (*
...
module In_memory_table : TABLE with type param := unit = struct
    type 'a table = {
        to_string : ('a -> string) ;
        of_string : (string -> 'a) ;
...
module Cache_table : TABLE with type param := string = struct
    (* Combined table type *)
    ...
    let init to_string of_string dir =
        ...
        let inmemory = In_memory_table.init to_string of_string () in
        (ondisk, inmemory)
